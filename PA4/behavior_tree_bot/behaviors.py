...
def attack_closest_weak_planet(state):
    if len(state.my_fleets()) >= 1:
        return False
    strongest_planet = max(state.my_planets(), key=lambda p: p.num_ships, default=None)
    
    arbitrary_planet_name = None
    arbitrary_planet_dist = 99999
    
    for planet in state.enemy_planets():
        dist = state.distance(strongest_planet.ID, planet.ID) + planet.num_ships()
        if dist < arbitrary_planet_dist:
            arbitrary_planet_name = planet
            arbitrary_planet_dist = dist
            
    if strongest_planet.num_ships > arbitrary_planet_name.num_ships + 15:
        issue_order(state, strongest_planet.ID, arbitrary_planet_name.ID, arbitrary_planet_name.num_ships + 15)
    else:
        return False